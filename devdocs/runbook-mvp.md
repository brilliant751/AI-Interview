...
2. 验证产物：
   ...
   - `data/chroma/kb_*/knowledge_index.jsonl`
   - `data/reports/*.json`
...
1. 本地/CI 执行质量门禁：
   - `python -m compileall backend/app scripts/data tests/backend`
...
4. 索引为空：确认已先执行 `normalize_materials.py`，并检查 `data/normalized/*_knowledge.jsonl` 是否存在。

## 8. 运维审计建议

1. 保留每次数据构建的 `data/reports/*.json`。
2. 发布单记录以下信息：
   - 代码版本
   - 执行人
   - 执行时间
