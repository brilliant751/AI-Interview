...
2. 验证产物：
   - `backend/assets/data/normalized/*.jsonl`
   - `backend/assets/data/sqlite/interview.db`
   - `backend/assets/data/chroma/kb_*/knowledge_index.jsonl`
...
4. 索引为空：确认已先执行 `normalize_materials.py`，并检查 `backend/assets/data/normalized/*_knowledge.jsonl` 是否存在。

## 8. 运维审计建议

1. 保留每次数据构建的 `backend/assets/data/reports/*.json`。
2. 发布单记录以下信息：
   - 代码版本
   - 执行人
   - 执行时间
